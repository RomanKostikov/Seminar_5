...
def player_vs_stupid_bot():
    ...
    while not win:
        ...
        if determing_moves >= count_for_check_win - 1:
            temp = methods.check_win(
                candies, determing_moves, name_player, 'Бот')
            if temp:
...
def player_vs_player():
    ...
    while not win:
        if determing_moves % 2 == 0:
            ...
            candies = methods.move_player(
                name_second_player, candies, max_move)
        if determing_moves >= count_for_check_win - 1:
            temp = methods.check_win(
                candies, determing_moves, name_first_player,
                name_second_player)
            if temp:
                print(f'{temp} выиграл(a)')
                win = True
        determing_moves += 1
